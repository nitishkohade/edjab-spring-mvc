...
import edjab.com.service.FacebookLogin;
...
public class FacebookLoginImpl implements FacebookLogin{
	...
	@Override
	public void loginViaFacebook(String facebookId, String facebookName, HttpServletRequest request,
		...
		try{
			...
			webResource = client.getClient().resource(serviceurl.getUrl()+"/userprofileservice/useridavailability/?userId="+facebookId);
			...
			String input = "{\"userId\":\""+facebookId+"\",\"password\":\"fbuserpassedstalkorg\",\"registeredUser\":\"false\",\"facebookUser\":\"true\",\"googleUser\":\"false\"}";
    		...
    		if(result.equals("true")){
    			...
    			getUserInfo(facebookId, facebookName, request, response, session);
    			
	    	if(session.getAttribute("facebook_user_emailId").equals("")){
	    		response.getWriter().print("200");
	    	}
	    	else{
	    		response.getWriter().print("200f");
	    	}
	...
	public void getUserInfo(String facebookId, String facebookName, HttpServletRequest request,	HttpServletResponse response, 
			HttpSession session){
		...
		webResource = client.getClient().resource(serviceurl.getUrl()+"/userprofileservice/?userId="+facebookId);
	    ...
	    if(object!=null){
	    	...
	    	session.setAttribute("isFacebookUser", "true");
	    	
	    	
	    	
	    	String emailId = object.has("emailId") ? object.get("emailId").getAsString() : "";
	    	session.setAttribute("facebook_user_emailId", emailId.equals(" ")?"":emailId);
	...
	session.setAttribute("user_emailId", facebookId);
	}

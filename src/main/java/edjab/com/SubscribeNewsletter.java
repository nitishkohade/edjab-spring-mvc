...
public class SubscribeNewsletter {
	...
	public void getSubscription_post( HttpServletResponse response, HttpServletRequest request, @RequestBody UpdateFrequency frequency) throws IOException{
		...
		//String input = "{\"emailId\":"+"\""+session.getAttribute("user_emailId")+"\""+",\"frequency\":"+"\""+frequency.getFrequency()+"\""+"}";
   	 
		String input = "{\"userId\":"+"\""+session.getAttribute("user_emailId")+"\""+",\"frequency\":"+"\""+frequency.getFrequency()+"\""+"}";
		
		webResource = client.getClient().resource(serviceurl.getUrl()+"/userprofileservice/updateSubscriptionFrequency/");
	 	...
	 	session.setAttribute("user_subscriptionFrequency", frequency.getFrequency().toUpperCase());

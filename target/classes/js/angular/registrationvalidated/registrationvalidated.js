angular.module('ngEdjab.registrationvalidated',[])

.factory('popupMessage', function($rootScope){
			...
			function popup(){
						var toolkit = new CookieToolkit( "Please wait while your registration is being validated!", "", "" );
			}
...
.controller('RegistrationValidated',['$scope', '$http', '$timeout', 'toaster', 'popupMessage', "ngProgressFactory", "GetContextPath", 
	...
	$scope.registrationResult = undefined;
			...
			$scope.$watch('token', function(newValue, oldValue){
				...
				if($scope.emailId!=null && $scope.token!=null){
					...
					$http.post(GetContextPath.getContextPath()+"/RegistrationValidated", input).success(function(result){
					...
					if(result=="200"){
						...
						$scope.registrationResult = "200";
			    		toaster.pop('success', "", '<ul class=\'list-inline\'><li style="font-variant: cursive; font-family: Courier, Times New Roman, Times, serif;">Your account is registered successfully</li></ul>', 5000, 'trustedHtml');
						}, 1000)
				...
				.error(function(result){
					...
					progress.reset();
					$timeout(function(){
					$scope.registrationResult = "error"
